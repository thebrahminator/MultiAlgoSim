...
typedef struct{
    uint64_t tier_1_read_hit;
    uint64_t tier_2_read_hit;
    uint64_t tier_1_write_hit;
    uint64_t tier_2_write_hit;
    uint64_t tier_1_read_miss;
    uint64_t tier_2_read_miss;
    uint64_t tier_1_write_miss;
    uint64_t tier_2_write_miss;
    uint64_t tier_1_total_hit;
    uint64_t tier_1_total_miss;
    uint64_t tier_2_total_hit;
    uint64_t tier_2_total_miss;
    uint64_t read_count;
    uint64_t write_count;
    uint64_t total_count;
    uint64_t tier_2_not_checked; // Only for exclusive caches, if there's a hit in layer 1. 
}traceStats;
...
void inclusive_caching(char* filename, uint64_t tier_1_size, uint64_t tier_2_size, char* op_fname){
    ...
    cache_t *tier_2_cache = create_cache("FIFO", tier_2_params, NULL);
    ...
    basic_stats_file(op_fname,multi_tier_stats,tier_1_size,tier_2_size,0);
...
void exclusive_caching(char *filename, uint64_t tier_1_size, uint64_t tier_2_size, char* op_fname){
    ...
    basic_stats_file(op_fname,exclusive_stats,tier_1_size,tier_2_size,1);
